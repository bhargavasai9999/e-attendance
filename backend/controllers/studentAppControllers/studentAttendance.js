...
export const markAttendance = async (req, res) => {
    ...
    try {
        ...
        const result = await database.query(query, [req.userId, token]);
        if(result.rows[0].attendance_status==='Present'){
            return res.status(200).json({message:"Attendance already marked"})
        }
        await database.query(`CALL updateAttendanceStatus()`);

        if (result.rowCount > 0) {
            const { checkin_time, checkout_time, attendance_status } = result.rows[0];

            if (checkin_time && checkout_time) {
                return res.status(200).json({ message: "Out Attendance Marked" });
            } else if (checkin_time) {
                return res.status(200).json({ message: "In Attendance Marked" });
            }
        }
